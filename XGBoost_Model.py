...
def create_lag_features(series, lags=[1, 2, 3, 5, 7, 14, 21]):
    """
    Enhanced feature engineering with more lag features and rolling statistics
    """
    ...
    for window in [7, 14, 30]:
        df[f'rolling_mean_{window}'] = df['total_attacks'].rolling(window=window).mean()
        df[f'rolling_std_{window}'] = df['total_attacks'].rolling(window=window).std()
        df[f'rolling_max_{window}'] = df['total_attacks'].rolling(window=window).max()
    
    df['diff_1'] = df['total_attacks'].diff()
    df['diff_7'] = df['total_attacks'].diff(7)
...
def train_xgboost_model(X_train, y_train):
    """
    XGBoost model with hyperparameters from XGBoost_with_news.py
    """
...
def plot_predictions(train, test, predictions, country_name):
    ...
    fig.update_layout(
        title=f'{country_name} - Cyber Attacks: Actual vs Predicted (XGBoost)',
        xaxis_title='Date',
...
def main():
    ...
    feature_columns = df_lags.columns.drop('total_attacks')
    df_lags[feature_columns] = scaler.fit_transform(df_lags[feature_columns])
